...
async function loadMyCompanies() {
  ...
  myCompanies = Object.entries(companies)
    .filter(([, c]) => c.owner === currentUserName)
    .map(([id, c]) => ({ id, ...c }))
    .sort((a, b) => (a.createdAt || 0) - (b.createdAt || 0));

  if (myCompanies.length === 0) {
    $('companyCreate').style.display = 'block';
    $('companyCreateNote').textContent = 'VIP会員のみ企業を作成できます';
    $('companyCreateNote').style.display = 'block';
    $('ownedCompaniesArea').style.display = 'none';
    $('hasCompany').style.display = 'none';
    return;
  }

  selectedCompanyId = myCompanies[0].id;
  $('companyCreate').style.display = myCompanies.length < 2 ? 'block' : 'none';
  $('companyCreateNote').style.display = 'block';
  $('ownedCompaniesArea').style.display = 'block';
...
function renderOwnedCompanies() {
  const list = $('ownedCompaniesList');
  if (!list) return;
  list.innerHTML = myCompanies.map(c => `
    <button class="btn-sm${c.id === selectedCompanyId ? ' active' : ''}" onclick="selectMyCompany('${c.id}')">${esc(c.name)}</button>
  `).join('');
}

window.selectMyCompany = function(id) {
...
function showMyCompany() {
  const c = myCompanies.find(company => company.id === selectedCompanyId) || myCompanies[0];
  if (!c) {
    $('companyCreate').style.display = myCompanies.length < 2 ? 'block' : 'none';
    $('companyCreateNote').style.display = 'block';
    $('ownedCompaniesArea').style.display = 'none';
  ...
  const companyCount = myCompanies.length;
  $('companyCreate').style.display = companyCount < 2 ? 'block' : 'none';
  $('companyCreateNote').textContent = companyCount === 1
    ? '2つ目の企業作成には100かむかかります'
    : companyCount === 0
      ? 'VIP会員のみ企業を作成できます'
      : '';
  $('companyCreateNote').style.display = 'block';
...
window.createCompany = async function() {
  ...
  if (currentUser && currentUser.trialVipExpireAt && currentUser.trialVipExpireAt > Date.now()) {
    msg.textContent = '無料VIPトライアル中は企業を作成できません'; return;
  }
  ...
  const snap = await get(ref(db, 'companies'));
  const companies = snap.exists() ? Object.values(snap.val()) : [];
  const ownedCount = companies.filter(c => c.owner === currentUserName).length;
  if (ownedCount >= 2) {
    msg.textContent = '企業は最大2つまでです'; return;
  }
  const creationFee = ownedCount === 1 ? 100 : 0;
  if ((currentUser.balance || 0) < creationFee) {
    msg.textContent = `2つ目の企業を作成するには${creationFee}かむ必要です`; return;
  }
  if (creationFee > 0) {
    const newBalance = (currentUser.balance || 0) - creationFee;
    const newHistory = [...(currentUser.history || []), `💼 2つ目の企業作成 -${creationFee} かむ`].slice(-10);
    await set(ref(db, `accounts/${currentUserName}`), {
      ...currentUser,
      balance: newBalance,
      history: newHistory
    });
  ...
  msg.textContent = creationFee > 0 ? '2つ目の企業を作成しました！' : '企業を作成しました！';
  $('companyName').value = '';
  $('initialPrice').value = '';
  $('initialShares').value = '';
  await loadMyCompanies();
...
window.addMember = async function() {
  ...
  if (!company) { msg.textContent = '企業が見つかりません'; return; }
  const members = company.members || [];
  if (members.length >= 4) { msg.textContent = 'メンバー上限（4人）に達しています'; return; }
  if (name === company.owner) { msg.textContent = 'オーナーは追加できません'; return; }
  ...
  await set(ref(db, 'companies/' + company.id + '/members'), [...members, name]);
  $('newMemberName').value = '';
...
window.issueShares = async function() {
  ...
  const company = myCompanies.find(c => c.id === selectedCompanyId);
  if (!company) { msg.textContent = '企業が見つかりません'; return; }
  const current = company.stock?.totalShares || 0;
  if (current + add > 1000) { msg.textContent = `発行上限は1000株です（現在${current}株）`; return; }
  const currentInv = company.stock?.inventory || 0;
  await set(ref(db, 'companies/' + company.id + '/stock/totalShares'), current + add);
  await set(ref(db, 'companies/' + company.id + '/stock/inventory'), currentInv + add);
  $('newShares').value = '';
...
window.dissolveCompany = async function() {
  ...
  const company = myCompanies.find(c => c.id === selectedCompanyId);
  if (!company) return;
  await set(ref(db, 'companies/' + company.id), null);
  await loadMyCompanies();
};
...
async function executeTrade(company) {
  ...
  try {
    ...
    const newPrice = Math.max(1, rawPrice - amount * 0.005);
    ...
    const newPrice = Math.max(rawPrice + amount * 0.01, 1);
    const hist = company.stock?.priceHistory || [];
    hist.push(price); if (hist.length > 20) hist.shift();
    await dbSet('companies/' + tradeCompanyId + '/stock/inventory', newInv);
    await dbSet('companies/' + tradeCompanyId + '/stock/price', newPrice);
  ...
  const rows = Object.entries(holdings).map(([id, shares]) => {
    ...
    return `<div class="holding-row">
      <div class="holding-info">
        ...
        <div class="holding-detail">${Math.round(shares)}株 × ${formatPrice(price)} かむ/株</div>
      </div>
