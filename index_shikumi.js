...
function hasNG(text) {
  const normalizedText = normalizeForNG(text);
  return NG_WORDS.some((word) => normalizedText.includes(normalizeForNG(word)));
}
...
function updatePremiumUI() {
  ...
  if (vip) {
    ...
    if (isTrialVip(currentUser)) {
      $("vipSince").textContent = `無料VIPトライアル期限: ${new Date(currentUser.trialVipExpireAt).toLocaleDateString("ja-JP")}`;
      $("vipTrialNote").style.display = "block";
    } else {
      ...
      $("vipTrialNote").style.display = "none";
    }
...
function isTrialVip(user) {
  if (!user) return false;
  return user.trialVipExpireAt && user.trialVipExpireAt > Date.now();
}

function isVip(user) {
  return isPaidVip(user) || isTrialVip(user);
}
...
window.createAccount=async function() {
  ...
  if(hasNG(name)){setMessage("message",TEXT.invalidName);return;}
...
window.startTrialVip = async function() {
  const msg = $("trialMsg");
  msg.style.color = "red";
  if (!currentUser) { msg.textContent = "ログインしてください"; return; }
  if (isVip(currentUser)) { msg.textContent = "既にVIPです"; return; }
  if (currentUser.trialVipUsed) { msg.textContent = "無料VIPトライアルは既に利用済みです"; return; }
  if (!confirm("1週間無料VIPを始めますか？無料VIP中は企業を作成できません。")) return;
  try {
    const now = Date.now();
    const newExpire = now + 7 * 24 * 60 * 60 * 1000;
    await update(ref(db, `accounts/${currentUserName}`), {
      trialVipExpireAt: newExpire,
      trialVipUsed: true
    });
    currentUser.trialVipExpireAt = newExpire;
    currentUser.trialVipUsed = true;
    updateBankUI();
    msg.style.color = "green";
    msg.textContent = "1週間無料VIPトライアルを開始しました";
  } catch {
    msg.textContent = "開始できませんでした";
  }
};
